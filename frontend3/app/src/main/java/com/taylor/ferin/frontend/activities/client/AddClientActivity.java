package com.taylor.ferin.frontend.activities.client;
...
public class AddClientActivity extends AppCompatActivity {
    ...
    public class UserRegisterTask extends AsyncTask<Void, Void, Login> {
        ...
        @Override
        protected com.taylor.ferin.frontend.domain.login.Login doInBackground(Void... params) {

            final String uri = "http://148.100.5.6:8080/login-request/";

            com.taylor.ferin.frontend.domain.login.Login user = new com.taylor.ferin.frontend.domain.login.Login.Builder()
                    .username(mEmail)
                    .password(mPassword)
                    .build();
            ...
            RestTemplate restTemplate = new RestTemplate();
            restTemplate.getMessageConverters().add(new GsonHttpMessageConverter());
            ResponseEntity<Login> result = restTemplate.exchange(uri, HttpMethod.POST, requestEntity, com.taylor.ferin.frontend.domain.login.Login.class);
            return result.getBody();
        }

        @Override
        protected void onPostExecute(final com.taylor.ferin.frontend.domain.login.Login success) {
            runOnUiThread(new Runnable() {
                public void run() {
